...
def get_new_events(cfg_hour_prior):
    ...
    for match in response["events"]:
        ...
        if timedelta(hours=0) <= time_difference <= timedelta(hours=cfg_hour_prior):
            ...
            match_date = correct_date_timestamp(start_time)
            ...
            print(f"Check duplicate result: {entry_exist}")
            
            add_match_to_database(mycursor, team1, team2, sofascore_matchpage_url, scraped, team1_score, team2_score, match_date)
